...
class RequestFacebookHelper
{
    public static function checkCookieLogin($username, $ch = false, $userAgent = false, $proxy = false)
    {
        ...
        $ch = $ch ? $ch : curl_init();
        $userAgent = $userAgent ? $userAgent : 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.93 Mobile Safari/537.36';
        $headers = array(
        ...
        $cookies = array();
        $debuginfo = "";
        $result = RequestHelper::curlExec($ch,$url,$headers,$cookies,$debuginfo);
//        dd($cookiePath);
        if (RequestHelper::isSuccess($ch) && $result) {
            if (strpos($result, 'error":1357032') !== false) {
                return true;
            }
        }
        return false;
    ...
    public static function checkLoginStatus($username, $password, &$ch = false, &$cookies = [], $userAgent = false, $proxy = false)
    {
        $cookiePath = Helper::getCookieFBFilePath($username);
        ...
        $result = RequestHelper::curlExec($ch,$url,$headers,$cookies,$debuginfo);
        if (RequestHelper::isSuccess($ch) && $result) {
            return self::getLoginStatus($result);
        }
        return false;
    }

    public static function getLoginStatus($html)
    {
